...
class Query {
    constructor(table, questions, columns) {
        ...
        this.joinQuery = 
            `SELECT e.id, e.first_name, e.last_name, e.role_id, role.name as title, role.salary, department.name as department, CONCAT_WS(" ", m.first_name, m.last_name) AS manager
            FROM employee e
